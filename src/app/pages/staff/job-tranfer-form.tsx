...
import { JobTranferSchema } from '../../../lib/form-schema';
...
const JobTranferForm = ({ user, open, setOpen }: JobTranferProps) => {
    const { data: clinics, isLoading } = clinicService.GetClinics();
    const { mutateAsync: putUser } = userService.PutUser();
    const form = useForm<z.infer<typeof JobTranferSchema>>({
        resolver: zodResolver(JobTranferSchema),
        defaultValues: {
            userName: user.userName,
            oldClinicId: user.clinicId
        },
    ...
    async function onSubmit(values: z.infer<typeof JobTranferSchema>) {
        try {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Chuyển công tác</DialogTitle>
                </DialogHeader>
                <FormProvider {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)}>
                        <div className="my-10 flex items-end justify-center gap-x-8 align-middle">
                            <FormField
                                control={form.control}
                                name="oldClinicId"
                                render={({ field }) => (
                                    <FormItem className="flex flex-col gap-y-1">
                                        <FormLabel>Phòng khám cũ</FormLabel>
                                        <FormControl>
                                            <Select
                            ...
                            />
                            <ArrowRightLeft className="h-8 w-8 rounded-md border border-primary p-2" />
                            <FormField
                                control={form.control}
                                name="clinicId"
                                render={({ field }) => (
